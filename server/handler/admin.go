...
func AdminHandler(w http.ResponseWriter, r *http.Request) {
	...
	subjectLabels := map[string]string{
		"stage": "Opportunité de stage", "projet": "Proposition de projet",
		"question": "Question", "autre": "Autre",
	}
...
h1{font-size:1.5rem;font-weight:800;color:var(--accent);margin-bottom:.5rem}
h2{font-size:1.1rem;font-weight:700;color:var(--text);margin:2rem 0 1rem;border-bottom:1px solid var(--border2);padding-bottom:.5rem}
.meta{font-size:11px;color:var(--muted2);margin-bottom:2rem}
.card{background:var(--surface);border:1px solid var(--border2);padding:1.2rem 1.4rem;margin-bottom:.75rem}
.card:hover{border-color:rgba(0,245,160,.2)}
.card-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:.75rem;flex-wrap:wrap;gap:8px}
.card-name{font-size:14px;font-weight:700}
.card-date,.card-id{font-size:10px;color:var(--muted2)}
.card-subject{font-size:10px;color:var(--accent);border:1px solid rgba(0,245,160,.3);padding:2px 8px}
.card-msg{font-size:13px;color:#c8d4e0;line-height:1.7;margin-top:.5rem}
.card-ip{font-size:10px;color:var(--muted2);margin-top:.5rem}
.card-mail{font-size:11px;color:var(--accent);margin-top:.25rem}
...
.stat-label{font-size:10px;color:var(--muted2);letter-spacing:.1em}
.back{display:inline-block;margin-top:2rem;font-size:12px;color:var(--accent);text-decoration:none}
.tag{font-size:10px;color:var(--accent);letter-spacing:.1em}
</style></head><body>
<p class="tag">~/admin $</p>
...
<h2>Messages de contact</h2>`, store.Total)
	...
	if len(store.Messages) == 0 {
		...
		for i := len(store.Messages) - 1; i >= 0; i-- {
			...
			label := subjectLabels[msg.Subject]
			if label == "" {
				label = msg.Subject
			}
			unread := ""
...
func AdminMarkReadHandler(w http.ResponseWriter, r *http.Request) {
	...
	store := model.MessagesStore{}
	data, err := os.ReadFile(model.MessagesFile)
	if err != nil {
	...
	os.WriteFile(model.MessagesFile, updated, 0644)
